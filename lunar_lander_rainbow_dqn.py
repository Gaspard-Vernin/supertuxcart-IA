...
class Noisy_linear_layer(nn.Module):
    ...
    def reset_bruit(self):
        ...
        return
class Net (nn.Module):
    def __init__(self,n,N=51):
        super(Net,self).__init__()
        self.nb_actions = 4
        self.taille_etat=8
        self.fc1=Noisy_linear_layer(self.taille_etat,128)
        self.fc2=Noisy_linear_layer(128,128)
        self.fc3A=Noisy_linear_layer(128,self.nb_actions*N)
...
net=Net(n)
goal_net=Net(n)
buffer=Buffer(capacity=10000,alpha=0.6,batch_size=64,beta=0.4)
...
for iter in range(500):
    ...
    while not done:
        with torch.no_grad():
            ...
            #on calcule les q values en faisant l'esperance pour tt les distributions calculées
